...
# cv2.dnn's forward pass otherwise grabs every core in its own thread pool for
# the duration of each inference call. On the Pi 4B's 4 cores that starves the
# LiDAR thread of scheduling time long enough for the OS serial buffer to
# overflow ("Too many bytes in the input buffer ... Cleaning buffer"), forcing
# an RPLidar rescan. Leaving 1-2 cores free keeps that thread - and the camera
# capture thread and the Flask dashboard - responsive while inference runs.
cv2.setNumThreads(2)
...
STEER_CMD_MIN = -140                # servo command at full left lock
...
THROTTLE_POLARITY = -1           # set to -1 if a positive command drives the
                                 # car backwards. Applied only in set_drive(),
...
# ---------------------------------------------------------------- 1.5 camera
# "/dev/webcam" is a stable symlink, not a real device - create it with a udev
# rule keyed to the webcam's vendor:product ID so it does not move to
# /dev/video1 after a reboot or a different USB port. Numeric index 0 also
# works but is not reliable once a second camera or capture device is plugged
# in.
CAMERA_DEVICE = "/dev/webcam"    # or an integer index such as 0
FRAME_WIDTH = 640
FRAME_HEIGHT = 480               # a mini USB 2.0 webcam is usually MJPG-only
                                 # at this size; raw YUYV caps out near 5 fps
...
# ---------------------------------------------------------- 1.8 ONNX detector
# Resolved next to this script, not the process's working directory, so this
# still finds best.onnx when launched by a systemd service or a start button
# instead of a manually cd'ed-into terminal (rule 9.10 / 9.11).
...
ONNX_INPUT_SIZE = 416            # confirmed from best.onnx's own input tensor
                                 # (images: [1,3,416,416]) - must always match
                                 # whatever imgsz a re-export uses
ONNX_CONF_THRESHOLD = 0.45
ONNX_NMS_IOU_THRESHOLD = 0.45
# Detection runs on its own thread at whatever pace inference allows (see
# Camera._detect_loop) rather than every Nth captured frame, so the video
# stream never stalls waiting for a slow forward pass.
...
LIDAR_MAX_RANGE_MM = 3200        # the mat is 3200 mm across (rule 13.1)
# Only the front-facing 180 deg (robot-frame -90 .. +90, i.e. left nose right)
# is kept - nothing behind the car matters for driving decisions, and every
# point dropped here is one less point to convert, store and percentile per
# scan, which matters when the CPU is already busy with camera + ONNX.
...
SECTOR_FRONT_WIDTH_DEG = 30      # total width of the "straight ahead" wedge
SECTOR_DIAGONAL_WIDTH_DEG = 40   # width of the front-left / front-right wedges
SECTOR_DIAGONAL_CENTRE_DEG = 50  # where those diagonals sit
SECTOR_SIDE_WIDTH_DEG = 40       # width of the pure left / right wedges

# ------------------------------------------------------- 1.11 driving control
DRIVE_CRUISE_SPEED = 170         # throttle while lane following (0-255)
DRIVE_CORNER_SPEED = 170        # INCREASED: Give motor torque to push through corners!
DRIVE_PILLAR_SPEED = 180        # throttle while passing a pillar
DRIVE_MIN_SPEED = 150            # motor needs at least 100 to push the car smoothly
...
FRONT_OBSTACLE_MM = 600          # EXACTLY 60cm for front triggering
SIDE_OBSTACLE_MM = 400           # EXACTLY 40cm for left/right triggering
ESCAPE_REVERSE_MM = 250          # Safe Mode: If closer than 25cm to front, go backward!

CORNER_EXIT_MM = 700             # Distance to return to straight cruising
...
class RobotLink:
    ...
    def _ramp_loop(self):
        ...
        while self.running:
            ...
            # "MOTOR LISTENS TO SERVO" - Drop throttle if servo is still turning!
...
class Camera:
    ...
    def _open(self):
        ...
        if not capture.isOpened() or not capture.read()[0]:
            capture.release()
            return None
        logging.info("[camera] opened %dx%d @ %.0f fps (asked for %dx%d - a cheap "
                     "USB camera can silently ignore the request, so check this)",
    ...
    def _loop(self):
        """Capture and stream only. Never blocks on the detector, so the
        video feed stays smooth at the camera's own frame rate no matter how
        slow ONNX inference is - see _detect_loop for that part."""
        ...
        while self.running:
            ...
            self._draw_overlay(frame, self.detections())
    ...
    def _detect_loop(self):
        """Runs on its own thread, at whatever pace inference allows, always
        on the freshest frame available - it drops frames rather than
        queuing them, so a slow model never builds up a backlog."""
        frames_this_second, second_started = 0, time.time()
        while self.running:
            with self._lock:
            ...
            with self._lock:
                ...
                if now - second_started >= 1.0:
                    self.detect_fps = frames_this_second / (now - second_started)
                    frames_this_second, second_started = 0, now
...
class Driver:
    ...
    def _decide(self, view, pillar):
        ...
        # This completely replaces the "stop" code so the robot is free.
        ...
        # 2. LiDAR sees something ahead - ask the camera what it is before
        # reacting blindly. A fresh, confident, in-range pillar colour always
        # wins here: RED means pass on the right, GREEN means pass on the
        # left (rule 9.19 via choose_pass_side), regardless of what the LiDAR
        # alone would have done. This is what makes the two sensors "listen"
        # to each other instead of the LiDAR overriding the camera.
        ...
        # 3. No usable colour from the camera - fall back to LiDAR-only
        # cornering/obstacle-avoidance to get away from whatever is ahead
        # (a wall, a track boundary corner, anything without a recognised
        # pillar colour).
        ...
        # 4. Nothing from the camera, nothing from the LiDAR corner check:
        # plain LiDAR corridor centring.
...
@app.route("/telemetry")
def telemetry():
    empty_camera = {"connected": False, "fps": 0, "detect_fps": 0, "backend": "none",
